...
export default function ProducerSettingsPage() {
  ...
  // Fetch profile to display company info
  const { data: profile, isLoading: profileLoading } = useQuery({
    queryKey: ["producer-profile-settings", user?.id],
    queryFn: async () => {
      if (!user?.id) return null
      const { data, error } = await supabase
        .from("producer_profiles")
        .select("id, company_name, company_type, verified")
        .eq("user_id", user.id)
        .maybeSingle()
    ...
    <div className="space-y-8 max-w-3xl">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Manage your security settings, notification preferences, and account details.
        </p>
      </div>

      <div className="space-y-6">
        {/* Panel 1: Account Info Card */}
        <div className="bg-card/25 border border-border/40 rounded-2xl p-6 backdrop-blur-md">
          <h2 className="text-base font-bold text-foreground flex items-center gap-2 mb-4">
            <Building className="h-4.5 w-4.5 text-brand-500" /> Company Account Details
          </h2>
          ...
          {profileLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
              <div>
                ...
                <p className="font-bold text-foreground mt-1 text-base">{profile.company_name}</p>
              </div>
              <div>
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Company Type</span>
                <p className="text-foreground mt-1 capitalize font-medium">{profile.company_type?.replace(/_/g, " ")}</p>
              </div>
              <div>
                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Account Email</span>
            ...
            <div className="text-sm text-muted-foreground flex items-center gap-1.5">
              ...
              <span>Could not fetch profile details. Please complete onboarding.</span>
            </div>
          )}
        </div>
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setNotifs(prev => ({ ...prev, newSubmissions: e.target.checked }))}
                  className="h-4 w-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500 accent-brand-500"
                />
                <div>
                  <span className="text-sm font-semibold text-foreground">New talent submissions</span>
                  <p className="text-[10px] text-muted-foreground">Receive instant alerts when talent apply to your casting calls</p>
                </div>
              </label>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={notifs.schedulingUpdates}
                  onChange={(e) => setNotifs(prev => ({ ...prev, schedulingUpdates: e.target.checked }))}
                  className="h-4 w-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500 accent-brand-500"
                />
                <div>
                  <span className="text-sm font-semibold text-foreground">Scheduling updates & confirmations</span>
                  <p className="text-[10px] text-muted-foreground">Get notified when a talent confirms an audition slot or uploads a self-tape</p>
                </div>
              </label>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={notifs.platformTips}
                  onChange={(e) => setNotifs(prev => ({ ...prev, platformTips: e.target.checked }))}
                  className="h-4 w-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500 accent-brand-500"
                />
                <div>
                  <span className="text-sm font-semibold text-foreground">Platform tips & news</span>
                  <p className="text-[10px] text-muted-foreground">Stay updated with feature announcements and producer guides</p>
                </div>
              </label>
            </div>
        ...
        <div className="bg-card/25 border border-border/40 rounded-2xl p-6 backdrop-blur-md space-y-6">
          <div>
            ...
            <p className="text-xs text-muted-foreground mb-4">Permanent changes to your Producer Space account.</p>
            ...
            {!deleteConfirmOpen ? (
              ...
              <div className="p-4 border border-destructive/30 bg-destructive/5 rounded-2xl max-w-md space-y-3">
                <p className="text-xs font-semibold text-destructive">
                  WARNING: This will deactivate your company profile and soft-delete your user account.
                </p>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {deleteLoading ? <Loader2 className="h-4.5 w-4.5 animate-spin" /> : "Confirm Deactivate"}
                  </button>
                  <button
                    type="button"
